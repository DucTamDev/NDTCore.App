...
const initUsbNative = (): Promise<void> =>
  new Promise((resolve, reject) => {
    NativeModules.RNUSBPrinter.init(
      () => resolve(),
...
/**
 * Native module `RNUSBPrinter` (com.ndtcorepos.thermalprinter) là singleton
 * dùng chung giữa `ThermalReceiptDriver` (escpos) và `TsplDriver` (tspl, qua
 * `UsbTransport`) — gọi `init()` 2 lần từ 2 driver độc lập sẽ đăng ký trùng
 * `BroadcastReceiver` ở tầng native (`USBPrinterAdapter.init()`). Memoize
 * promise ở đây để toàn app chỉ `init()` đúng 1 lần bất kể driver nào gọi trước.
 */
export const ensureUsbInitialized = (): Promise<void> => {
  if (!initPromise) initPromise = initUsbNative();
  return initPromise;
};
