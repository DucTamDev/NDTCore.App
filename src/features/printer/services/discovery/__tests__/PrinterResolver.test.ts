...
describe('resolveIdentityKey', () => {
  ...
  it('builds a usb:<vid>:<pid> key for USB when no serial is available', () => {
    expect(resolveIdentityKey({ connectionType: ConnectionType.usb, device: usbDevice })).toBe('usb:1155:22222');
  });
  ...
  it('pins the serial into the key when the USB descriptor carries one', () => {
    ...
    expect(resolveIdentityKey({ connectionType: ConnectionType.usb, device: withSerial })).toBe('usb:1155:22222:XPR-000123');
  });
