...
- `CutElement { type: 'cut'; options?: CutOptions }` — real behavior. `CutOptions` already exists (`builder/printer/CutElement.ts`): `{ rows?: number; mode?: 'off' | 'partial' | 'full' }`. New `PrintBuilder.cut(options: CutOptions = {}): this` method.
  ...
  - `TscCompiler`: emit `SET CUTTER` per mode — `'off'` → `SET CUTTER OFF`, `'full'`/`'partial'` → `SET CUTTER <rows ?? 1>` (verified against this repo's own `src/features/printer/drivers/tspl/TsplEncoder.ts`'s `cut()` method, which uses the bare `SET CUTTER <n>` form with no `BATCH` keyword — an earlier draft of this spec incorrectly guessed a `BATCH,` prefix; corrected 2026-09-15 after Task 4's implementer caught the discrepancy against the real reference file). Add the `SET CUTTER`-family constant(s) to `TscCommand.ts`.
